...
func (coll *HistColl) GetChunkOfSample() *chunk.Chunk {
	...
	for i, key := range mapKey {
		chunk.SetCol(int(i), coll.Columns[key].SampleCol.SampleColumn)
	}
	return chunk
}
...
func analyzeSample(ctx sessionctx.Context, histColl *HistColl, columnID int64, isIndex bool, sampleSize uint64, fulltable bool) error {
	...
	for {
		...
		if result.IsIndex == 1 {
			histColl.Indices[result.Sample[0].SID].SampleCol = result.Sample[0]
		} else {
			...
			for _, samplec := range result.Sample {
				...
				histColl.Columns[samplec.SID].SampleCol = samplec
...
func (e *AnalyzeSampleExec) buildSample() (sample []*SampleCol, err error) {
	if RandSeed == 1 {
...
func (e *AnalyzeSampleExec) runTasks() ([]*SampleCol, error) {
...
func (e *AnalyzeSampleExec) buildColSample(colID int64, collector *SampleCollector, isPk bool) (*SampleCol, error) {
...
func (e *AnalyzeSampleExec) buildIdxSample(colID int64, collector *SampleCollector) (*SampleCol, error) {
	// TODO
